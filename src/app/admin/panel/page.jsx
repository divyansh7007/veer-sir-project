...
function Page() {
  ...
  const onRemoveAdmin = async (id) => {
    setLoading(true);
    try {
      const data = await fetch(`/api/user/removeAdmin/${id}`, {
        method: "POST",
        cache: "no-cache",
      }).then((r) => r.json());
      getData();
  ...
  const onAddAdmin = async (id) => {
    setLoading(true);
    try {
      const data = await fetch(`/api/user/addAdmin/${id}`, {
        method: "POST",
        cache: "no-cache",
      }).then((r) => r.json());
      getData();
  ...
  return (
    <section className="text-gray-600 body-font">
      ...
      <div className="text-center flex items-center justify-center">
        ...
        >
          Manage Premium Membership <AiOutlineArrowRight className="text-xl" />{" "}
        </Link>
      </div>
    </section>
  );
